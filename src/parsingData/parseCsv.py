        ...
        for row in reader:
            if prev_product != row[filterKey]:
                ...
                if len(entities) % 100 == 0:
                    print(
                        f"Detected {str(len(entities))} new Entities with filter key [{filterKey}]"
                    )
...
def parse1():
    data_dict = parseCsv(
        "./data/csv/DatafinitiElectronicsProductData.csv",
        [
            "name",
            "brand",
            "categories",
    ...
    for row in data_dict:
        ...
        for category_name in category_names:
            checked_categories = (
                session.query(Category).filter(Category.name == category_name).all()
            )
            if len(checked_categories) == 0:
                ...
                ins = links_products_categories.insert().values(
                    product_id=new_product.id, category_id=checked_categories[0].id
                )
                session.execute(ins)
            links_count += 1
        session.commit()
