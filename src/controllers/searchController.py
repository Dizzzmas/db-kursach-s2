...
class SearchController(object):
    def getProductsByCostRange(self, min: int, max: int, *args):
        try:
            if len(args) == 0:
                return (
                    session.query(Product.id, Product.name, Product.cost)
    ...
    def getAllClientOrders(self, client_id: int):
        try:
            return (
                session.query(Order.id, Order.transaction_date, Order.taxes_sum)
    ...
    def getAllProductCategories(self, product_id: int):
        try:
            return (
                session.query(
                    Product.id,
                    func.array_agg(Product.name),
                    func.array_agg(Category.name),
                )
