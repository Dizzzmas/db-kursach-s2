...
class ModelController(object):
    ...
    def delete(self, itemId):
        try:
            ...
            if deletedItem is None:
                raise Exception("This id doesn't exists")
    ...
    def update(self, item):
        try:
            ...
            session.query(self.instance).filter(self.instance.id == item.id).update(
                self.getModelEntityMappedKeys(item)
            )
            session.commit()
            return True
        except Exception as err:
    ...
    def getCount(self):
        try:
            return session.execute(
                select([func.count()]).select_from(self.instance)
            ).scalar()
        except Exception as err:
    ...
    def getModelKeys(self):
        ...
        for entity in self.instance.__dict__.items():
            ...
            key_type = entity[1]
            if (
                type(key_type) is InstrumentedAttribute
                and key is not "id"
