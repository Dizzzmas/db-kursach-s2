...
class SearchView:
    def __init__(self):
        ...
        self.CUI.addField(
            "Search products cost by cost range",
            lambda: self.__getProductsByCostRange(),
        )
        self.CUI.addField("Search all client orders", lambda: self.__getClientOrders())
        self.CUI.addField(
            "Search all product categories", lambda: self.__getCategoryProducts()
        )
    ...
    def __getProductsByCostRange(self):
        ...
        try:
            if self.min is None or self.max is None:
                ...
                if (
                    not isinstance(self.min, int)
                    and not isinstance(self.max, int)
                    and self.max > 0
                    and 0 < self.min <= self.max
                ):
    ...
    def __getCategoryProducts(self):
        ...
        try:
            ...
            searchMenu.addField(
                f"Product id: {allRecords[0][0]}  |   Product name: {allRecords[0][1][0]}\nCategories:"
            )
