...
class EntityView:
    ...
    def __supportFillItemFunc(self, key, value, item):
        try:
            ...
            self.currentMenu.renameField(
                f"{key}:        <{value}>", f"{key}:        <{new_value}>"
            )
            ...
            if len(args) > 0 and isinstance(args[0], self.instance):
                ...
                for (key, value) in self.Controller.getModelEntityMappedKeys(
                    ...
                    self.currentMenu.addField(
                        f"{key}:        <{value}>",
                        lambda key=key, value=value: self.__supportFillItemFunc(
                            key, value, item
                        ),
                    )
                ...
                for key in self.Controller.getModelKeys():
                    mapped[key] = None
                    self.currentMenu.addField(
                        f"{key}", lambda key=key: self.__supportFillFunc(key, mapped)
                    )
            else:
        ...
        try:
            ...
            for value in mapped.values():
                if value is None or (
                    ...
                    and any((char in exec_bad_chars) for char in value)
                ):
    ...
    def __getItems(self):
        ...
        try:
            ...
            if self.page < count:
                itemsMenu.addField(
                    "NEXT",
                    lambda: self.__changePageParams(self.page + 1, self.per_page),
                )
            if self.page > 1:
                itemsMenu.addField(
                    "PREV",
            ...
            for entity in entities:
                if "name" in self.Controller.getModelKeys():
                    ...
                    itemsMenu.addField(
                        f"<{entity.id}>", lambda id=entity.id: self.__getItem(id)
                    )
