...
engine = create_engine(f"postgresql://localhost/db_course_work")
...
def defineGenerateStringFunc():
    session.execute(
        "create or replace function generateString(length int) "
        "returns text "
        "language plpgsql "
        "as "
        "$$ "
...
def defineGenerateDateFunc():
    session.execute(
        "create function generatedate() returns date "
        "language plpgsql "
        "as "
        "$$ "
        "declare "
...
def defineGenerateIntFunc():
    session.execute(
        "create function generateint(max integer) returns text "
        "language plpgsql "
        "as "
        "$$ "
        "declare "
...
def defineGetRandowRowFunc():
    session.execute(
        "create or replace function getrandomrow(table_name text) returns text "
        "language plpgsql "
        "as "
        "$$ "
        "declare "
