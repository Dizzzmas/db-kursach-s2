...
def getTop15Categories():
    try:
        results = (
            session.query(Category.name, func.count(Category.name).label("count"))
...
def getManufactureDateStat():
    results = (
        session.query(
            func.extract("year", Product.manufacture_date).cast(Integer).label("year"),
            func.count("year"),
        )
        .group_by("year")
        .order_by("year")
...
def getTotalMoneyPerYear():
    results = (
        session.query(
            func.sum(Product.cost).label("sum"),
            func.extract("year", Order.transaction_date).cast(Integer).label("year"),
        )
        .join(Product.Orders)
        .group_by("year")
